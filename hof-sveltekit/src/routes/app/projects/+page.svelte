<script lang="ts">
  ...
  async function loadProjects() {
    try {
      ...
      // Load all projects (lab members can see all projects)
  ...
  function updateStats() {
    ...
    projectStats.backlog = projects.filter(p => p.status === 'Backlog').length;
    projectStats.inProgress = projects.filter(p => p.status === 'In Progress').length;
    projectStats.done = projects.filter(p => p.status === 'Done').length;
  }
  ...
  $: filteredProjects = projects.filter(project => {
    ...
    const matchesOwner = !ownerFilter || project.createdBy === ownerFilter;
...
<svelte:head>
  <title>Projects - CPX Lab</title>
  <meta name="description" content="Manage and collaborate on research projects" />
</svelte:head>
  ...
  <div class="bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div class="text-center">
        <h1 class="text-4xl font-bold text-gray-900 mb-4" style="font-size: var(--text-4xl);">Projects</h1>
        <p class="text-xl text-gray-600 mb-8" style="font-size: var(--text-xl);">Manage and collaborate on research projects</p>
  ...
  <div class="bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div class="stats-card">
          <div class="text-3xl font-bold text-gray-900 mb-2">{projectStats.backlog}</div>
          <div class="text-sm text-gray-600">Backlog</div>
        </div>
        <div class="stats-card">
          <div class="text-3xl font-bold text-gray-900 mb-2">{projectStats.inProgress}</div>
          <div class="text-sm text-gray-600">In Progress</div>
        </div>
        <div class="stats-card">
          <div class="text-3xl font-bold text-gray-900 mb-2">{projectStats.done}</div>
          ...
          {#each filteredProjects as project}
            <div class="project-card">
              <div class="flex items-start justify-between mb-4">
                ...
                <div class="flex items-center space-x-2">
                  {#if project.createdBy === $user.uid}
                    <i class="fas fa-crown text-yellow-500" title="Project Owner"></i>
                  {/if}
                </div>
              </div>
              ...
              <div class="flex items-center justify-between mb-3">
                ...
                <a href="/app/projects/{project.id}" class="text-gray-600 hover:text-gray-900 font-medium" style="font-size: var(--text-sm);">
                  View Details
                </a>
              </div>
                ...
                <div class="flex items-center space-x-2">
                  ...
                  {#if project.links.overleafUrls?.length > 0}
                    <span class="inline-flex items-center px-2 py-1 rounded text-xs bg-green-100 text-green-800">
                      <i class="fas fa-file-code mr-1"></i>Overleaf
                    </span>
                  {/if}
